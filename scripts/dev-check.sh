...
check() {
  ...
  local url="$2"
  local code
  code="$(curl -s "${CURL_OPTS[@]}" -o /dev/null -w '%{http_code}' "$url" || echo "000")"
  if [[ "$code" =~ ^2 ]]; then
    echo "OK   $name ($code)"
  else
...
code="$(curl -s "${CURL_OPTS[@]}" -o /dev/null -w '%{http_code}' "$API_URL/api/sessions/${SESSION_KEY}/track-asset" || echo "000")"
...
if command -v jq >/dev/null 2>&1; then
  status_json="$(curl -sf "${CURL_OPTS[@]}" "$API_URL/api/ingestion/status" 2>/dev/null || echo '{}')"
  ...
  playback_json="$(curl -sf "${CURL_OPTS[@]}" "$API_URL/api/sessions/${SESSION_KEY}/playback" 2>/dev/null || echo '{}')"
...
play_code="$(curl -s "${CURL_OPTS[@]}" -o /dev/null -w '%{http_code}' -X POST \
